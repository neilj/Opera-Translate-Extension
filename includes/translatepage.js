...
function createTransactions ( target ) {
	...
	function chunkTarget () {
		...
		while ( ( textnode = iterator.nextNode() ) ) {
			len += overheadPerItem +
			    encodeURIComponent( textnode.textContent ).length;
...
function translate ( transactionId, reset ) {
	...
	var transaction = transactions[ transactionId ],
	    translatedStrings = ( reset ? 
            transaction.originalStrings : transaction.translatedStrings ),
        l = translatedStrings.length,
        nodes = transaction.nodes,
        temp = document.createElement( 'div' );
    
  	for ( var i = 0, l = nodes.length; i < l; i += 1 ) {
   		nodes[i].textContent = decodeEntities( translatedStrings[i], temp );
   	}
    ...
    if ( transactionsReceived === initialTransactionsExpected ) {
        ...
        label.textContent =
            opera.extension.i18n.getMessage('translation_complete_status', [ opera.extension.i18n.getMessage( fromLang.code ) ]);
        translateButton.textContent = opera.extension.i18n.getMessage('translation_revert_language_button');
        cancelButton.textContent = opera.extension.i18n.getMessage('translation_close_bar_button');
...
function nodeInserted ( event ) {
	...
	if ( event.target.id === '_opera_extension_$_popup_statusbar_' ||
			event.target.id === '_opera_extension_$_popup_translatehover_') {
		return;
	}
    ...
    div.appendChild(
		optionsSelect = Element.create( 'select', {
            ...
            Element.create( 'option', {
                text: opera.extension.i18n.getMessage('translate_option_always', [ opera.extension.i18n.getMessage( fromLang.code ) ]),
                style: 'padding: 2px 0;',
                value: 'always',
                selected: data.preference === 'always' ? 'selected' : null
            }),
            Element.create( 'option', {
                text: opera.extension.i18n.getMessage('translate_option_ask', [ opera.extension.i18n.getMessage( fromLang.code ) ]),
                style: 'padding: 2px 0;',
                value: 'ask',
                selected: data.preference === 'ask' ? 'selected' : null
            }),
            Element.create( 'option', {
                text: opera.extension.i18n.getMessage('translate_option_never', [ opera.extension.i18n.getMessage( fromLang.code ) ]),
                style: 'padding: 2px 0;',
                value: 'never',
            ...
            Element.create( 'option', {
                text: opera.extension.i18n.getMessage('translation_bar_options_message'),
                style: 'padding: 2px 0;',
                value: 'more'
            })
        ] 
    ...
    translateButton.addEventListener( 'click', function () {
        ...
        if ( isTranslated ) {
        	...
        	document.removeEventListener(
        	    'DOMNodeInserted', nodeInserted, false );
        	
        	for ( i = 0, l = transactions.length; i < l; i += 1 ) {
        		translate( i, true );
    		}
            cleanup();
            hideMessage();
        } else {
            for ( i = 0, l = initialTransactions.length; i < l; i += 1 ) {
                var transaction = initialTransactions[i];
        		if ( !transaction.translatedStrings.length ) {
		    		opera.extension.postMessage( {
		                action: 'translate',
		                data: {
		                	id: transaction.id,
            ...
            label.textContent =
            	opera.extension.i18n.getMessage('translation_in_progress_status', [ opera.extension.i18n.getMessage( fromLang.code ) ]) + "…";
