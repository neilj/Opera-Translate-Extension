...
function chunkDOMTree ( target ) {
    var iterator = getIterator( target ),
        chunks = [],
        overheadPerItem = 3, // translation overhead per item '&q='
        strings,
        nodes,
        segments,
        len,
        textnode, textLength, id;
    ...
    while ( true ) {
        ...
        while ( ( textnode = iterator.nextNode() ) ) {
            ...
            textLength = encodeURIComponent( textnode.data ).length;
            ...
            if ( textLength <= 3950 ) {
                ...
                // Google Translate chunking limits
                if ( len > 4000 || segments === 100 ) {
                    // Back up one place; we'll process this next time.
                    iterator.previousNode();
                    break; 
                }
                
                textnode.isChunked = true;
                nodes.push( textnode );
...
function requestTranslations( node ) {
    var chunks = chunkDOMTree( node ),
        chunk, i, l;
...
function translatePage ( reset ) {
    if ( reset ) {
        for ( var i = 0, l = transactions.length; i < l; i += 1 ) {
            translate( i, true );
        }
        hideMessage();
        cleanup();
    } else {
        requestTranslations( document.body );
        if ( div ) {
            label.textContent =
                'Translating page from ' + fromLang.value + "…";
...
function translate ( id, reset ) {
    var chunk = transactions[ id ],
        translatedStrings = reset ? 
            chunk.originalStrings : chunk.translatedStrings,
        l = translatedStrings.length,
        nodes = chunk.nodes,
        temp = document.createElement( 'div' );
    
    while ( l-- ) {
        nodes[l].data = decodeEntities( translatedStrings[l], temp );
    }
    if ( !reset ) {
        chunksReceived += 1;
    }
    if ( chunksReceived === transactions.length ) {
        // Update bar to say it is translated.
        if ( div ) {
            label.textContent =
                'This page has been translated from ' + fromLang.value;
...
function showMessage ( data ) {
    ...
    div = Element.create( 'div', {
        ...
        label = Element.create( 'span', {
            style: 'color: inherit; padding: 0; padding-left: 34px; margin: 0 8px 0 0;',
            text: 'This page appears to be written in ' + fromLang.value + '.'
        }),
    
        translateButton = Element.create( 'button', {
            text: 'Translate',
            style: 'color: inherit; padding: 1px 5px; margin: 0 8px 0 0; border: 1px solid #888; border-radius: 5px; background: #eee url(' + btnGrad + ') repeat-x; display: inline; text-shadow: 0 1px 0 #ddd;font: 12px/20px "Lucida Grande", Arial, sans-serif;'
        }),
        cancelButton = Element.create( 'button', {
            text: 'No thanks',
            style: 'color: inherit; padding: 1px 5px; margin-right: 0 8px 0 0; border: 1px solid #888; border-radius: 5px; background: #eee url(' + btnGrad + ') repeat-x; display: inline; text-shadow: 0 1px 0 #ddd;font: 12px/20px "Lucida Grande", Arial, sans-serif;'
        }),
        optionsSelect = Element.create( 'select', {
            ...
            Element.create( 'option', {
                text: 'Never ask to translate ' + fromLang.value,
                style: 'padding: 2px 0;',
                value: 'never',
      ...
      closeButton = Element.create( 'div', {
          text: '+',
          style: '-o-transform: rotate(45deg); border-radius: 9px; position: absolute; right: 10px; top: 7px; text-align: center; width: 17px; padding-right: 1px; height: 18px; color: #EEE; font: 14px/18px "Lucida Grande", Arial, sans-serif; cursor: pointer; background-color: #666;' 
      })
    ]);
    ...
    html.style.OTransition = 'padding-top 0.3s ease-in-out';
    html.appendChild( div );
    
    setTimeout( function() {
        html.style.paddingTop = '34px';
        div.style.top = '0';
    }, 0 );
}
...
( function () {
    ...
    getContent( 'h1, h2, h3, h4, h5, h6' );
    if ( text.length < 200 ) {
        getContent( 'p' );        
    }
    if ( text.length < 200 ) {
        getContent( 'ul, ol, table, nav, header, footer' );
    }
    if ( text.length >= 200 ) {
        opera.extension.postMessage({
            action: 'analyse',
...
// 2. Make sure all option elements have a value attribute.
// Otherwise they will use the text content inside.
( function () {
...
opera.extension.addEventListener( 'message', function( message ) {
    ...
    switch ( message.data.action ) {
        ...
        case 'translate':
            var id = data.id;
            transactions[ id ].translatedStrings = data.translatedStrings;
            translate( id );
            break;
        case 'fail':
            fail( data.id );
